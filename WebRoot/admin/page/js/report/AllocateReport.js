/***************************************
分配清单  AllocateReport.js
****************************************/

Ext.define('AllocateReport', {
...
Ext.define('Redm.report.AllocateReportGrid', {
    extend: 'Redm.BaseGrid',
    alias : 'widget.allocateReportgrid',
    loadMask: true,
    autoLoad:false,
    forceLayout:true,
   buildColumns: function(){
        this.columns = [
			...
			{ header: "客户订单号", dataIndex: 'externorderkey', width: 100, sortable: true},
		    ...
		    { header: "批次", dataIndex: 'lot', width: 150, sortable: true},
		    { header: "库位", dataIndex: 'loc', width: 80, sortable: true},
			{ header: "已分配数量", dataIndex: 'qty', width: 150, sortable: true,renderer:Ext.util.Format.numberRenderer('0.000')},
			{ header: '重量',dataIndex: 'lottable15',width: 110,sortable: true},
			{ header: '面积',dataIndex: 'udf1',width: 110,sortable: true,renderer:Ext.util.Format.numberRenderer('0.000')},
		    { header: '收货日期',dataIndex: 'lottable01',width: 110,sortable: true,editor:{xtype:'datefield',format:'Y-m-d H:i'},renderer : Ext.util.Format.dateRenderer('Y-m-d H:i')},
		    ...
		    { header: '失效日期',dataIndex: 'lottable03',width: 110,sortable: true,editor:{xtype:'datefield',format:'Y-m-d H:i'},renderer : Ext.util.Format.dateRenderer('Y-m-d H:i')},
...
//        });
                ...
                items: [
                   ...
                   '-',
					...
					'-',
					{
						iconCls: 'icon-upload',
                        text: '导出',
                        handler: this.onPrintPickDetailItem1, 
                        scope: this
                    }
    ...
    initComponent: function(){
    	var me = this;
    	this.buildStore(basePath + '/report/doQueryAllocateReportInfo.action','AllocateReport',20);
    ...
    onSelect: function(){
		...
		var storedKeyValue=father.selpanel.getForm().findField('storerKey').getValue();
        if(''==storedKeyValue)
        {
    ...
    onQueryQtySum:function(){
        ...
        //计算总数量，有问题，待解决
        var values = me.selpanel.getForm().getFieldValues();
        ...
        var editDataStart = values.editDataStart;
        var editDataEnd = values.editDataEnd;
        ...
        var externorderkey = values.externorderkey;
        
        Ext.Ajax.request({
                    url: basePath + '/report/doQueryAllocateReportInfoSum.action',
                    params: {
                        orderKey:orderKey,
                        storerKey:storerKey,
                        type:type,
                        ...
                        editDataStart:editDataStart,
                        editDataEnd:editDataEnd, 
	...
	onPrintPickDetailItem1: function(){
		...
		var editDataStart = Ext.util.Format.date(values.editDataStart,'Y-m-d H:i:s');
		var editDataEnd = Ext.util.Format.date(values.editDataEnd,'Y-m-d H:i:s');
    	var lottable01 = Ext.util.Format.date(values.lottable01,'Y-m-d H:i:s');
        var lottable01Over = Ext.util.Format.date(values.lottable01Over,'Y-m-d H:i:s');
        var lottable02 = Ext.util.Format.date(values.lottable02,'Y-m-d H:i:s');
        var lottable02Over = Ext.util.Format.date(values.lottable02Over,'Y-m-d H:i:s');
		...
		//导出excel格式的报表
		window.location.href = basePath+'/report/allocateReportPOIExcel.action?string='+storerKey+","+orderKey+","+type+","+status+","+sku
		                       +","+editDataStart+","+editDataEnd+","+lottable01+","+lottable02+","+lottable03+","+lottable04
							   +","+lottable05+","+lottable06+","+lottable07+","+lottable08+","+lottable09+","+lottable10+","
							   +lottable11+","+lottable12+","+lottable13+","+lottable14+","+lottable15+","+lottable16+","+externorderkey+","+lottable01Over+","+lottable02Over+","+lottable03Over
                                                                            +","+retailReference+","+buyerpo+","+carrierReference+","+consigneeCompany+","+carrierKey
                                                                            +","+carrierCompany+","+consigneeKey+","+model;

    }
});

Ext.define('Redm.report.AllocateReportTab', {	
	extend: 'Ext.panel.Panel',
    alias : 'widget.allocateReportmanager',
    title:'分配清单',
    layout:'border',
    initComponent: function(){
    	var me = this;
    ...
    createGridPanel:function(){
		var me = this;
    	this.gridPanel = Ext.create('widget.allocateReportgrid',{
			region: 'center'
		});
		this.gridPanel.getStore().on({
           beforeload:{fn:function(store){
    		...
    		var editDataStart = record.editDataStart;
			var editDataEnd = record.editDataEnd;
		
			var retailReference = record.retailReference;
    		var buyerpo = record.buyerpo;
			var carrierReference = record.carrierReference;
			
			var consigneeCompany = record.consigneeCompany;
    		var carrierKey = record.carrierKey;
    		var carrierCompany = record.carrierCompany;
			var model = record.model;
			var consigneeKey = record.consigneeKey;
			
			var lottable01 = Ext.util.Format.date(record.lottable01,'Y-m-d H:i:s');
            var lottable01Over = Ext.util.Format.date(record.lottable01Over,'Y-m-d H:i:s')
            var lottable02 = Ext.util.Format.date(record.lottable02,'Y-m-d H:i:s')
            var lottable02Over = Ext.util.Format.date(record.lottable02Over,'Y-m-d H:i:s')
            var lottable03 = Ext.util.Format.date(record.lottable03,'Y-m-d H:i:s')
            var lottable03Over = Ext.util.Format.date(record.lottable03Over,'Y-m-d H:i:s')
			...
			var externorderkey = record.externorderkey;
			...
			delete params.type;
			delete params.editDataStart;
			delete params.editDataEnd;
			...
			if(editDataStart) params.editDataStart = editDataStart;
			if(editDataEnd) params.editDataEnd = editDataEnd;
			...
			if(externorderkey) params.externorderkey = externorderkey;
    	   },scope: this},
    	     //计算本页总数量
            load:{fn:function(store){
                //此处在本页添加四个值(Lee)
                    ...
                    var var2Total = store.sum('lottable15');

...
Ext.define('Redm.support.SkuGrid', {
    ...
    buildColumns: function(){
        this.columns = [
		    ...
		    { header: "体积", dataIndex: 'cube', width: 100, sortable: true},
                    ...
                    items: [
                        {
                            ...
                            allowBlank: false,
                            name: 'storerKey',
							forceSelection: true,//true 将会限定选择的值是列表中的值之一
                            listeners:{
                                blur: function(txt){
								...
								select : function(txt){
                                    //输入参数，鼠标离开后见检查该商品是否存在
                                    storerKeyValue=txt.getValue();
									skuTypeValue= me.basicForm.getForm().findField('skuType').getValue('');
                                    if(''!=storerKeyValue &&null!=storerKeyValue && ''!=skuTypeValue&&null!=skuTypeValue) 
                                    {
                                        Ext.Ajax.request({
                                            url: basePath + '/support/doValidateSkuType.action',   //用true or false判断有误，后续再查，改用长度判断
                                            params: {
                                                storerKey:storerKeyValue,
                                            ...
                                            success: function(response){
                                                ...
                                                if(0==text.json.data.length)
                                                {
													...
													me.basicForm.getForm().findField('skuType').setValue('');
                                                }else{
													me.customerForm.getForm().findField('replenishmentStrategyKey').setValue(text.json.data[0].replenishmentStrategyKey);
													me.customerForm.getForm().findField('putawayStrategyKey').setValue(text.json.data[0].putawayStrategyKey);
													me.customerForm.getForm().findField('lotKey').setValue(text.json.data[0].lotKey);
													me.customerForm.getForm().findField('rotationStrategyKey').setValue(text.json.data[0].rotationStrategyKey);
													me.customerForm.getForm().findField('preAllocationStrategyKey').setValue(text.json.data[0].preAllocationStrategyKey);
													me.customerForm.getForm().findField('allocationStrategyKey').setValue(text.json.data[0].allocationStrategyKey);
												}
                                            }
                ...
                }, 
                {
                    ...
                    items: [
                                    ...
                                    if(''!=packKeyValue)
                                    {
                                        Ext.Ajax.request({
                                            ...
                                            success: function(response){ 
                                                ...
                                                {
                                                    MessageBox.show(false, '包装不存在');
                                                    me.basicForm.getForm().findField('rfPack').setValue('');                                                         
                                                }
                                            }
                                        })
                                    }
                        ...
                        },                        
                        {
                            ...
                            listeners: {   //选中包装后,
                              	expand : function(){
                                    packKeyValue=me.basicForm.getForm().findField('rfPack').getValue();

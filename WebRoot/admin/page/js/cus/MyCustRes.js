...
Ext.define('ContactRecord', {
    extend: 'Ext.data.Model',
    fields: [
		{name:'id'},
		{name:'cusId'},            
		{name:'userId'},    
		{name:'cantactDate',type:'date',dateFormat : 'Y-m-d H:i:s.u'},  
		{name:'nextDate',type:'date',dateFormat : 'Y-m-d H:i:s.u'},  
		{name:'remark'},  
		{name:'addDate',type:'date',dateFormat : 'Y-m-d H:i:s.u'}, 
		{name:'addWho'},
		{name:'editDate',type:'date',dateFormat : 'Y-m-d H:i:s.u'},     
...
Ext.define('Redm.cus.CustomerGrid', {
    ...
    initComponent: function(){
    	var me = this;
    	this.buildStore(basePath+'/cus/doQueryCustomer.action','Customer',20);
        this.callParent(arguments);
    }
});

Ext.define('Redm.cus.ContactRecordGrid', {
    extend: 'Redm.BaseGrid',
    alias : 'widget.contactRecordgrid',
    loadMask: true,
    forceLayout:true,
    dockedItems: [],
    buildColumns: function(){
        this.columns = [
		    { header: "客户ID", dataIndex: 'cusId', width: 90, sortable: true},
		    { header: "用户ID", dataIndex: 'userId', width: 90, sortable: true},
		    { header: "联系日期", dataIndex: 'cantactDate', width: 90, sortable: true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')},
		    { header: "预约下次时间", dataIndex: 'nextDate', width: 90, sortable: true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')},
    ...
    initComponent: function(){
    	var me = this;
    	this.buildStore(basePath+'/cus/doQueryContactRecord.action','ContactRecord',20);
        this.callParent(arguments);
    }
});
...
/*    		listeners:{
    ...
    createBtmPanel: function(){
    	this.btmpanel = Ext.create('Ext.panel.Panel',{
    		...
    		layout: 'border',
    		border: false,
    		items:[this.createLeftBtmPanel(),this.createForm()]
    	});
    	return this.btmpanel;
    },
    
    createLeftBtmPanel: function(){
    	this.btmpanel = Ext.create('Ext.panel.Panel',{
    		region: 'center',
    		layout: 'border',
    		border: false,
    		items:[this.createTopGridPanel(),this.createBottomGridPanel()]
    	});
    	return this.btmpanel;
    },
    ...
    createTopGridPanel:function(){
     ...
     //底部GRID
    createBottomGridPanel:function(){
    	var me = this;
    	this.bottomgridPanel = Ext.create('widget.contactRecordgrid',{
			region: 'center',
			listeners: {
    			itemclick: function(grid,record){
    				me.form.getForm().loadRecord(record);
		...
		this.bottomgridPanel.getStore().on('beforeload',function(){
    		var params = this.bottomgridPanel.getStore().getProxy().extraParams;
    		var record = me.selform.getForm().getValues();
